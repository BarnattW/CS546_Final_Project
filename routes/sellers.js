...
router
  ...
  .get(async (req, res) => {
    ...
    try {
      ...
      let alreadyReviewed = false; // Initialize as false
      let alreadyCommented = false; // Initialize as false

      // Check if the user has already reviewed this listing
      if (listing.reviews.length !== 0) {
        for (let i = 0; i < listing.reviews.length; i++) {
          if (listing.reviews[i].customerId === req.session.user._id) {
      ...
      // Check if the user has already commented on this listing
      if (listing.comments.length !== 0) {
        for (let i = 0; i < listing.comments.length; i++) {
          if (listing.comments[i].commenterId === req.session.user._id) {
      ...
      // Render the page based on whether the user has commented or reviewed
