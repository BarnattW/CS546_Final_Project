...
function validateItemName(str, varName) {
  let errors = [];
  try {
      str = validateString(str, varName);
  } catch (e) {
  ...
  if (str.length > 100) {
      errors.push(`${varName || "Item Name"} must not exceed 100 characters!`);
  }
...
function validateItemDescription(str, varName) {
  let errors = [];
  try {
      str = validateString(str, varName);
  } catch (e) {
  ...
  if (str.length > 500) {
      errors.push(`${varName || "Item Description"} must not exceed 500 characters!`);
  }
...
function validateItemPrice(price, varName) {
  let errors = [];
  try {
      validateNumber(price, varName);
  } catch (e) {
      errors = errors.concat(e);
  }

  if (price <= 0) {
      errors.push(`${varName || "Item Price"} must be greater than 0!`);
  }
